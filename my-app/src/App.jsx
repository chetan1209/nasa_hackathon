...
Cesium.Ion.defaultAccessToken =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiIyYjhjYmQ0YS1mMjJiLTRkZmUtYjg3OS0yNGQ4Y2EwNzNhMTYiLCJpZCI6MzQ3MDIzLCJpYXQiOjE3NTk1NTEzOTN9.CT2n4lChu6SwyXuoxjd_sxRTOsPrKz-d4KLTk5P-ca0"; // ❗️ PASTE YOUR TOKEN HERE
  ...
  new Promise((resolve) =>
    setTimeout(() => {
      ...
      const greenCount = items.filter(
        (i) => i.type === "vegetation" || i.type === "water"
      ).length;
      ...
      resolve({
        overall_score: score,
        categories: {
          sustainability: Math.min(100, 70 + greenCount * 4 + energyCount * 2),
        ...
        summary:
          score > 85
            ? "Excellent sustainability improvements!"
            : "Consider adding more green spaces or renewable energy.",
      });
    }, 500)
  );
...
const transformBackendData = (backendData) => {
  ...
  // Define baseline averages and standard deviations for conversion
  const BASELINES = {
    temperature: { mean: 21, stdDev: 3 }, // Mean of 21°C (~70°F)
    aqi: { mean: 50, stdDev: 15 },         // Mean AQI of 50 (Good/Moderate)
    water_vapour: { mean: 65, stdDev: 8 }, // Mean humidity of 65%
  };

  Object.keys(overall_scores).forEach((regionId) => {
    // Z-scores from your JSON
    ...
    const overallZScore = overall_scores[regionId] || 0;

    // Convert Z-scores to 0-100 scale for internal scoring
    const healthScore = Math.max(0, Math.min(100, 50 + overallZScore * 25));

    // --- NEW: Convert Z-scores to realistic raw values ---
    const rawTemperature = BASELINES.temperature.mean + tempZScore * BASELINES.temperature.stdDev;
    const rawAqi = Math.max(0, BASELINES.aqi.mean + aqiZScore * BASELINES.aqi.stdDev);
    const rawHumidity = Math.max(0, Math.min(100, BASELINES.water_vapour.mean + humidityZScore * BASELINES.water_vapour.stdDev));

    transformed[regionId] = {
      health_score: Math.round(healthScore),
      name: `Region ${regionId.replace('R', '')}`, // Cleaner name
      color_code: color_codes[regionId] || "yellow",
      // Add the newly calculated raw values
      raw_aqi: parseFloat(rawAqi.toFixed(0)),
      raw_temperature: parseFloat(rawTemperature.toFixed(1)),
      raw_humidity: parseFloat(rawHumidity.toFixed(0)),
    };
  });

  console.log("✅ Transformed data with generated raw values:", transformed);
  return transformed;
};
...
const Guide = ({ currentStep, setStep, refs }) => {
  ...
  const steps = [
    {
      title: "Welcome!",
      text: "This guide will walk you through the Urban Canvas AI.",
      target: null,
    },
    {
      title: "Design Palette",
      text: "Drag items like trees or solar panels from this panel directly onto the 3D map.",
      target: "palettePanel",
    },
    {
      title: "3D Viewport",
      text: "When you drag an item over the map, an area will be highlighted. Drop it to place it and see the impact.",
      target: null,
    },
    {
      title: "Urban Score",
      text: "As you make changes, our AI analyzes your design's impact. Watch your score change!",
      target: "scorePanel",
    },
    {
      title: "You're All Set!",
      text: "Now you're ready to start designing a greener Chicago. Enjoy!",
      target: null,
    },
  ];
  useEffect(() => {
    if (currentStep === null || !steps[currentStep].target) {
      setArrowProps(null);
      return;
    }
    const calculateArrow = () => {
      ...
      if (!guideBoxRef.current || !targetRef?.current) {
        setArrowProps(null);
        return;
      }
      const fromRect = guideBoxRef.current.getBoundingClientRect();
      const toRect = targetRef.current.getBoundingClientRect();
      const isTargetLeft = toRect.left < fromRect.left;
      setArrowProps({
        x1: isTargetLeft ? fromRect.left : fromRect.right,
        y1: fromRect.top + fromRect.height / 2,
        x2: isTargetLeft ? toRect.right + 10 : toRect.left - 10,
        y2: toRect.top + toRect.height / 2,
      });
    };
    calculateArrow();
    window.addEventListener("resize", calculateArrow);
    return () => window.removeEventListener("resize", calculateArrow);
  }, [currentStep, refs]);
  ...
  return (
    <div className="guide-overlay">
      ...
      <div ref={guideBoxRef} className="guide-box" style={positionStyle}>
        {" "}
        <button onClick={() => setStep(null)} className="close-button">
          &times;
        </button>{" "}
        <h3
          style={{
...
const animateFallingEntity = (viewer, entity, startHeight, duration = 2000) => {
  ...
  const animate = () => {
    ...
    const currentHeight =
      startCartographicWithHeight.height +
      (endCartographic.height - startCartographicWithHeight.height) *
        easeProgress;
...
const ChicagoUrbanPlanner = ({ onStartGuide }) => {
  ...
  const getGlobeCoordinatesFromMovement = useCallback((movement) => {
    ...
    const pickedPosition = viewer.scene.pickPosition(
      ...
      (async () => {
        try {
          ...
          const [regionGeometry, backendData, tileset] = await Promise.all([
            fetch("http://localhost:5001/api/regions")
              .then((res) => {
            ...
            fetch("http://localhost:5001/api/polygon-scores")
              .then((res) => {
                if (!res.ok) throw new Error("API not available");
                return res.json();
              })
              .catch(() =>
                fetch("/polygon_scoring_results.json").then((res) => res.json())
              ),
  ...
  useEffect(() => {
    ...
    if (hiddenAreas.length > 0) {
      const hideConditions = hiddenAreas.map(
        (area) =>
          `!((${'${feature["cesium#longitude"]}'} > ${
            area.west
          } && ${'${feature["cesium#longitude"]}'} < ${
            area.east
          }) && (${'${feature["cesium#latitude"]}'} > ${
            area.south
          } && ${'${feature["cesium#latitude"]}'} < ${area.north}))`
      );
...
const handleDragLeave = useCallback(() => {
  const viewer = viewerRef.current;
  if (highlightBlockRef.current && viewer?.entities) {
    // Remove inner ellipse if it exists
    if (highlightBlockRef.current._innerEllipse) {
      viewer.entities.remove(highlightBlockRef.current._innerEllipse);
    }
...
const handleMapDragOver = useCallback(
  (e) => {
    ...
    const cartographic = getGlobeCoordinatesFromMovement({
    ...
    // Remove old highlight if item type changed
    if (highlightBlockRef.current && 
        highlightBlockRef.current.name !== `Highlight ${draggedItem.id}`) {
      viewer.entities.remove(highlightBlockRef.current);
      highlightBlockRef.current = null;
    }

    if (!highlightBlockRef.current) {
      if (draggedItem.id === "trees") {
        // Tree outline - simple cylinder with cone on top
        highlightBlockRef.current = viewer.entities.add({
          name: `Highlight ${draggedItem.id}`,
      ...
      } else if (draggedItem.id === "water_body") {
        // Pond outline - double ellipse effect using two cylinders
        const outerEllipse = viewer.entities.add({
          name: `Highlight ${draggedItem.id}`,
          position: Cesium.Cartesian3.fromDegrees(
            centerLon,
            centerLat,
        ...
        // Inner ellipse
        const innerEllipse = viewer.entities.add({
          name: `Highlight ${draggedItem.id}_inner`,
          position: Cesium.Cartesian3.fromDegrees(
            centerLon,
            centerLat,
            groundHeight + 5
        ...
        highlightBlockRef.current = outerEllipse;
        highlightBlockRef.current._innerEllipse = innerEllipse;
        
      } else if (draggedItem.id === "cool_roof") {
        // Thin reflective surface - flat box
        highlightBlockRef.current = viewer.entities.add({
          name: `Highlight ${draggedItem.id}`,
          position: Cesium.Cartesian3.fromDegrees(
            centerLon,
          ...
          box: {
            dimensions: new Cesium.Cartesian3(60, 60, 4),
      ...
      if (draggedItem.id === "trees") {
        highlightBlockRef.current.position = Cesium.Cartesian3.fromDegrees(
          centerLon,
          centerLat,
          groundHeight + 75
        );
        ...
        if (highlightBlockRef.current._innerEllipse) {
          highlightBlockRef.current._innerEllipse.position = Cesium.Cartesian3.fromDegrees(
            centerLon,
            centerLat,
            groundHeight + 5
          );
...
const handleDrop = useCallback(
  async (e) => {
    ...
    if (targetRegion) {
      ...
      setPreviousScores(regionalScores); // Save current scores for UI comparison

      const oldRegionData = regionalScores[targetRegion.id];
      const oldScore = oldRegionData.health_score;
      ...
      const newRawTemp = oldRegionData.raw_temperature - (item.impactFactor * 0.2);
      const newRawHumidity = Math.min(100, oldRegionData.raw_humidity + (item.impactFactor * 0.5));
      ...
      setRegionalScores((prevScores) => ({
        ...
        [targetRegion.id]: {
          ...
          // Update the raw values in the state as well
          raw_aqi: parseFloat(newRawAqi.toFixed(0)),
          raw_temperature: parseFloat(newRawTemp.toFixed(1)),
          raw_humidity: parseFloat(newRawHumidity.toFixed(0)),
        },
      }));

      setImpactAnalysis({
        title: `Impact in Region ${targetRegion.id.replace('R','')}`,
      ...
      setImpactAnalysis({
        ...
        summary: "The item was placed outside of a designated analysis region.",
        airQuality: "+0.0 pts",
        propertyValue: "+0.0%",
      });
    }
    ...
    if (!finalPosition) {
      const [terrainSampledPos] = await Cesium.sampleTerrainMostDetailed(
    ...
    } else { // Rooftop or other single-entity items
      let positionWithOffset = finalPosition;
      if (isRooftopPlacement) {
        const cartoPos = Cesium.Cartographic.fromCartesian(finalPosition);
        cartoPos.height += 0.5; // Add a small offset to prevent z-fighting
        positionWithOffset = Cesium.Cartographic.toCartesian(cartoPos);
      }
      const entity = customDataSource.entities.add({
        id: `${placementId}_0`,
        model: { uri: item.modelUri, scale: item.scale },
  ...
  [
    ...
    history,
    regionGeoJson,
    regionalScores, // Add this to the dependency array
  ]
);
  ...
  const handleUndo = () => {
    ...
    if (lastAction.hiddenArea) {
      setHiddenAreas((prev) =>
        prev.filter(
          (area) => area.placementId !== lastAction.hiddenArea.placementId
        )
      );
    }
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = "chicago-design.json";
    a.click();
 ...
 const currentAiScoreDisplay = React.useMemo(() => {
  if (selectedRegionId && regionalScores && regionalScores[selectedRegionId]) {
    ...
    return {
      ...
      categories: {
        AQI: { value: regionData.raw_aqi, unit: "" },
        Temperature: { value: regionData.raw_temperature, unit: "°C" },
        Humidity: { value: regionData.raw_humidity, unit: "%" },
      },
      summary: 
          regionData.health_score >= 70
            ? "This region boasts excellent environmental health!"
            : regionData.health_score >= 40
            ? "Moderate environmental health. Consider improvements for a greener future."
            : "This region requires significant environmental improvements.",
      regionName: regionData.name,
      colorCode: regionData.color_code,
    };
  }
  // This was aiScore before, but null is better when nothing is selected
  return null; 
}, [selectedRegionId, regionalScores]); // Removed aiScore from dependencies
    ...
    <>
      <div className="planner-container">
        ...
        <div style={{
          position: 'absolute',
          inset: 0,
          background: 'linear-gradient(to right, transparent, rgba(255, 0, 0, 0.02), transparent)',
          transition: 'transform 1s ease',
          transform: isHovered ? 'translateX(100%)' : 'translateX(-100%)',
          pointerEvents: 'none',
          width: '30%'  // Add this line to make it narrower
        }} />
          {/* Button content */}
          <div
            style={{
          ...
          >
            ...
            <span
              style={{
                fontWeight: "600",
                background:
                  "linear-gradient(to right, #4ade80, #22c55e, #10b981)",
            ...
            <div
              style={{
                ...
                inset: 0,
                background:
                  "linear-gradient(to right, transparent, rgba(255, 255, 255, 0.01), transparent)",
                transition: "transform 1s ease",
                transform: isHovered ? "translateX(100%)" : "translateX(-100%)",
                pointerEvents: "none",
              }}
        ...
        >
          ...
          <div className="panel-actions">
            ...
            <button onClick={handleExport} className="action-button secondary">
              Export Design
            </button>
          </div>
        </DraggablePanel>
        ...
        {showAiScorePanel && currentAiScoreDisplay && (
          ...
          >
            ...
            >
              <div
                key={scoreKey}
                className="score-value pulse-glow"
                style={{
                  color: getColorStringForScore(
            ...
            >
{Object.entries(currentAiScoreDisplay.categories).map(
    ([key, data]) => {
      // --- ARROW LOGIC ---
      const prevData = previousScores?.[selectedRegionId];
      let arrow = <span style={{ color: THEME.textSecondary }}>-</span>; // Default: no change
      let valueColor = THEME.textPrimary;

      if (prevData) {
        const oldValue = prevData[`raw_${key.toLowerCase()}`];
        const newValue = data.value;

        // Determine arrow direction and color
        if (newValue < oldValue) {
          // Good change (temp/aqi down) or bad change (humidity down)
          const isGoodChange = key === 'Temperature' || key === 'AQI';
          arrow = <span style={{ color: isGoodChange ? THEME.primary : "rgb(231, 76, 60)" }}>↓</span>;
          valueColor = isGoodChange ? THEME.primary : "rgb(231, 76, 60)";
        } else if (newValue > oldValue) {
          // Bad change (temp/aqi up) or good change (humidity up)
          const isGoodChange = key === 'Humidity';
          arrow = <span style={{ color: isGoodChange ? THEME.primary : "rgb(231, 76, 60)" }}>↑</span>;
          valueColor = isGoodChange ? THEME.primary : "rgb(231, 76, 60)";
        }
      }
      ...
      return (
        <div key={key} className="category-card" style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <span style={{ fontSize: "16px", fontWeight: "bold", width: '12px', textAlign: 'center' }}>
              {arrow}
            </span>
            <div className="category-value" style={{ fontSize: "22px", color: valueColor }}>
              {/* --- DISPLAY RAW VALUE AND UNIT --- */}
...
const App = () => {
  ...
  return (
    <div
      style={{ width: "100vw", height: "100vh", background: THEME.background }}
    >
      <style>{`
...
@keyframes fadeInStagger { 
    ...
    to { 
      ...
      {view === "landing" ? (
        <LandingPage onStart={startPlanner} />
      ) : (
        <>
          {" "}
          <ChicagoUrbanPlanner onStartGuide={startGuide} />{" "}
          <Guide
